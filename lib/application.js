...
module.exports.Server = (cfg = {}) => async function ({ router, identity }) {
  ...
  router.get(cfg.route.login, async function (ctx) {
    ...
    if (!valid) {
      await ctx.render('pages/login', { _csrf: ctx.state.csrf, action: cfg.route.login + '?' + ctx.querystring })
    } else {
  ...
  router.post(cfg.route.login, async function (ctx) {
    try {
      ...
      if (ret) {
        ...
        await ctx.render('pages/login', { _csrf: ctx.state.csrf, message: 'user does not exist or password does not match' })
      }
    } catch (error) {
      await ctx.render('pages/login', { _csrf: ctx.state.csrf, message: error.message })
    }
  })
  ...
  router.get(cfg.route.home, async function (ctx, next) {
    await ctx.render('index', { _csrf: ctx.state.csrf })
  })
}
